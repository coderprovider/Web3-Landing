  ...
  return (
    <div>
      <div className="transition-all py-2 min-h-[250px] flex flex-col justify-start px-5 bg-plugin-bg rounded-2xl opacity-80">
        ...
        <h2 className="mt-2 md:mt-7 text-gray-100 text-2xl font-semibold leading-7">
          {name}
        </h2>
        {description ? (
          <h3 className="mt-2 text-gray-300 text-md leading-6">
            {description}
          </h3>
        ) : null}
      </div>
      {!error && (
        <div className="flex justify-between mt-2 mx-2">
          <div className="flex flex-col">
            {authors && (
              <p className="text-lg text-gray-50 font-semibold mb-1">
                {authors}
              </p>
            )}
